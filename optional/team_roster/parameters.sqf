...
GVAR(parameters) = [
	...
	999999,					// 2  - CycleTime - Time between each update of the list in seconds (DEFAULT: 300)
	"Platoon Roster",			// 3  - SubjectText - Text to be shown as the subject in the Map Screen(DEFAULT: "Team Roster")
	...
	"platoon_roster_units",                 // 9  - UnitArray - String of command for all units to be included in Roster (DEFAULT: "call CBA_fnc_players")
	[],                     		// 10 - SkippedUnits - Array of all units NOT to be included in Roster regardless of side (DEFAULT: [])
	"Platoon Roster",                       // 11 - HeaderText - Text to be shown above the Team Roster, ":SIDE:" represent's player's side (DEFAULT: "Team Roster (:SIDE:)")
	"2",					// 12 - FireteamColor - Coloring of fireteams on the Team Roster, "0" = off, "1" = player's squad only, "2" = all units (DEFAULT: "1")
