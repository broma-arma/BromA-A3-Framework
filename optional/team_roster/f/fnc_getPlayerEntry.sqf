...
if _playerLeader then {
    if (_isMedic == 1) then {
        _string = "<br/>" + _string + _playerRank + _playerName + " (" + _playerDesc + ")" + " (<img image='optional\team_roster\f\med_icon.paa' width='16' heigth='16' />)"; // "Alpha Squad - Sgt. Crackman (Squad Leader)" or "    Pvt. Crackman (Machinegunner)"
    } else {
    ...
    if (_isMedic == 1) then {
        _string = _string + _playerRank + _playerName + " (" + _playerDesc + ")" + " (<img image='optional\team_roster\f\med_icon.paa' width='16' heigth='16' />)"; // "Alpha Squad - Sgt. Crackman (Squad Leader)" or "    Pvt. Crackman (Machinegunner)"    
    } else {
