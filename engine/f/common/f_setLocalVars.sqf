...
// Sets up the platoon roster units

pre_var_units = allUnits + vehicles;
platoon_roster_units = [];
{if ((side _x) == side player) then { platoon_roster_units = platoon_roster_units + [_x]}} forEach pre_var_units;

// =============================================================================

// IF THE MISSION IS NOT A TVT THEN WE WAIT FOR DAC
...
#define SLV_NAME "("+ENGINE_PATH+"f\common\f_setLocalVars.sqf)"
...
// CREATES VARS FOR POINT ASSESSMENT

f_var_units_PLAYERSIDE = [];
{if ((side _x) == missionPlayerSide) then {f_var_units_PLAYERSIDE = f_var_units_PLAYERSIDE + [_x]}} forEach f_var_units;

f_var_units_ENEMYSIDE = [];
{if ((side _x) == missionEnemySide) then {f_var_units_ENEMYSIDE = f_var_units_ENEMYSIDE + [_x]}} forEach f_var_units;
...
if (isServer) then { f_var_init = 1; publicVariable "f_var_init" };
diag_log "== PLAYER: ALL LOCAL VARIABLES INITIALIZED ===========================";