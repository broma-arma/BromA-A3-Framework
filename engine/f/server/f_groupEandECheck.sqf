...
{
	player sideChat format ["DEBUG (engine\f\server\f_groupEandECheck.sqf): _alive = %1",_alive];
};
...
if (f_var_debugMode == 1) then
{
	player sideChat format ["DEBUG (engine\f\server\f_groupEandECheck.sqf): _safe = %1",_safe];
};

if (_alive == _safe) exitWith {myEnd1 = [_end] execVM "engine\f\server\f_mpEndBroadcast.sqf";};
_safe = 0;
