...
{
    {
        if ((alive _x)&&(!([_x] call fnc_isDead))) exitWith { KEGs_target = _x };
    } forEach allUnits;
};
...
spectate_events = compile preprocessFileLineNumbers format ["%1spectator\specta_events.sqf",ENGINE_path];
RefreshPlayerList = compile preprocessFileLineNumbers format ["%1spectator\RefreshPlayerList.sqf",ENGINE_path];
FreeLookMovementHandler = compile preprocessFileLineNumbers format ["%1spectator\FreeLookMovementHandler.sqf",ENGINE_path];
CameraMenuHandler = compile preprocessFileLineNumbers format ["%1spectator\CameraMenuHandler.sqf",ENGINE_path];
PlayerMenuHandler = compile preprocessFileLineNumbers format ["%1spectator\PlayerMenuHandler.sqf",ENGINE_path];
KEGsShowCombatMode = compile preprocessFileLineNumbers format ["%1spectator\specta_combatmode.sqf",ENGINE_path];
KEGsShowUnitLocator = compile preprocessFileLineNumbers format ["%1spectator\specta_locator.sqf",ENGINE_path];
...
if ([player] call fnc_isDead) then {
